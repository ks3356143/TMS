      ...
      <div class="left-side">
        <div class="panel">
          <a-form :model="DemandSearch" layout="inline">
            <a-form-item>
              <a-select :style="{width:'180px'}" :loading="loading" placeholder="请选择" @search="SearchProduct" :filter-option="false">
                <a-option v-for="item of options" :value="item">{{item}}</a-option>
              </a-select>
            </a-form-item>
            <a-form-item>
              <a-input
                v-model="DemandSearch.productId"
                allow-clear
                placeholder="所属项目"
              />
            </a-form-item>
            ...
            <a-form-item>
              <a-input
                v-model="DemandSearch.content"
                allow-clear
                placeholder="需求内容模糊搜索"
              />
            </a-form-item>
            <a-form-item>
              <a-input
                v-model="DemandSearch.begintime"
                allow-clear
                placeholder="需求内容模糊搜索"
              />
            </a-form-item>
        ...
        >
          <a-form ref="formRef" :model="DemandForm" :rules="rules">
            <a-form-item field="keyCode" label="项目编号" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.keyCode" placeholder="keycode不可重复"  allow-clear />
            </a-form-item>
            <a-form-item field="title" label="项目名称" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.title" placeholder="项目线名称" allow-clear />
            </a-form-item>
            <a-form-item field="type" label="项目类型" :validate-trigger="['change','input','focus']">
              <a-select v-model="DemandForm.type" placeholder="请选择项目类型" allow-clear>
            ...
            <a-form-item field="tester" label="项目负责人" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.tester" placeholder="请输入项目负责人，TODO：以后改为热搜索" allow-clear />
            </a-form-item>
            <a-form-item field="step" label="测评阶段" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.step" default-value="测评大纲编写" placeholder="请输入测评阶段，默认开始" allow-clear />
            </a-form-item>
            <a-form-item field="customer" label="客户单位" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.customer" placeholder="请输入客户单位" allow-clear />
            </a-form-item>
            <a-form-item field="seller" label="销售人员" :validate-trigger="['change','input','focus']">
              <a-input v-model="DemandForm.seller" placeholder="请输入项目对应销售人员，TODO：改为热搜索" allow-clear />
            </a-form-item>
          </a-form>
        </a-modal>
        <a-modal
        ...
        >
        <a-form :model="DemandForm" :rules="rules">
          <a-form-item field="keyCode" label="项目编号" :validate-trigger="['change','input','focus']">
            <a-input v-model="DemandForm.keyCode" placeholder="keycode不可重复"  allow-clear />
          </a-form-item>
          <a-form-item field="title" label="项目名称" :validate-trigger="['change','input','focus']">
...
const SearchProduct = (value) => {
      if (value) {
        loading.value = true;
        window.setTimeout(() => {
          options.value = [`${value}-Option1`, `${value}-Option2`, `${value}-Option3`] //设置选项
          loading.value = false;
        }, 2000)
      } else {
        options.value = []
...
const SetInitProductForm = () => {
  DemandForm.id = undefined;
  DemandForm.type = undefined;
  DemandForm.keyCode = undefined;
  DemandForm.title = undefined;
  DemandForm.tester = undefined;
  DemandForm.seller = undefined;
  DemandForm.step = '测评大纲编写';
  DemandForm.customer = undefined;
};
...
const pageChange = (current: number) => {
  console.log(current);
  DemandSearch.currentPage = current;
  btnSearchClick();
};
const pageSizeChange = (pageSize: number) => {
  console.log(pageSize);
  DemandSearch.pageSize = pageSize;
  btnSearchClick();
};
const btnSearchClick = async () => {
  // const res = await apiDemandSearch(productSearch);
  const res = await apiDemandSearchPage(DemandSearch);
  if (res.code === 20000) {
...
const addModalOk = async () => {
    // 首先初始化对象
        ...
        const res = await apiDemandAdd(productForm);
        if (res.code === 20000) {
...
const editButtonClick = (record) => {
  // 修改项目线按钮触发事件
  DemandForm.id = record.id;
  DemandForm.type = record.type;
  DemandForm.keyCode = record.keyCode;
  DemandForm.title = record.title;
  DemandForm.tester = record.tester;
  DemandForm.step = record.step;
  DemandForm.customer = record.customer;
  DemandForm.seller = record.seller;
  editModalVisible.value = true; // 编辑显隐赋值为True显示
};
const editModalOk = async () => {
  // 编辑对话框确定按钮，提交数据操作
  const res = await apiDemandUpdate(DemandForm);
  if (res.code === 20000) {
...
const removeButtonClick = async (id) => {
  const res = await apiDemandRemove(id);
  if (res.code === 20000) {
...
const delModalOk = async () => {
  const res = await apiDemandRemove(delId.value);
  if (res.code === 20000) {
